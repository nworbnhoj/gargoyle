...
set_version_variables()
{
	...
	lower_short_gargoyle_version=$(echo "$short_gargoyle_version" | tr '[:upper:]' '[:lower:]' )
...
#'
	{
		printf "| %-26s| %-35s|" "Gargoyle version $gargoyle_version" "$openwrt_branch_str"
		printf "| %-26s| %-35s|" "Gargoyle revision $gargoyle_commit" "OpenWrt revision r$openwrt_revision"
		printf "| %-26s| %-35s|" "Built $date" "Target  $target/$profile"
		echo '------------------------------------------------------------------'
	} >> "$banner_file_path"
...
distrib_copy_arch_ind_ipk()
{
	...
	local dpkgs
	dpkgs=$(find "$top_dir/package" -path '*plugin-gargoyle-*' -and -name 'Makefile' -and -not -path '*-i18n-*' -print0 | xargs grep -s -l "DEPENDS:=+gargoyle$" | xargs grep -s -l "PKGARCH:=all$" | awk -F'/' '{print $(NF-1)}')
	...
	while true; do
		local apkg
		apkg=$(echo "$dpkgs" | awk -v rec=$di 'NR==rec {print $0}')
		[[ -z "$apkg" ]] &&
		{
			break
		} || {
...
download_openwrt()
{
		...
		cd "$branch_id" || exit
		find . -name ".svn" -print0 | xargs -r rm -rf
		cd "$top_dir" || exit
...
for target in $targets ; do
	...
	find . -name ".*sw*" 	-print0 | xargs rm -rf
...
# build only profiles specied by parameters, or build all profiles
	...
	for profile in $profiles ; do
		...
		openwrt_target=$(grep "^CONFIG_TARGET_BOARD=" .config | sed 's/\"$//g' | sed 's/^.*\"//g')
		...
		make "$num_build_threads" GARGOYLE_VERSION="$numeric_gargoyle_version" GARGOYLE_VERSION_NAME="$lower_short_gargoyle_version" GARGOYLE_PROFILE="$profile"
		...
		#	rm -rf "staging_dir"
			...
			profile_images=$(cat "$targets_dir/$target/profiles/$profile/profile_images" 2>/dev/null)
			for pi in $profile_images ; do
				candidates=$(find "bin/$arch/" "*$pi*" 2>/dev/null | sed 's/^.*\///g')
