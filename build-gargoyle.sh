...
for target in $targets ; do
	...
	if [ "$build_openwrt" = true ] ; then # clobber all files from prior builds
		rm -rf "$target-src/*"
	elif [ -e "$target-src/bin" ] ; then # just clobber packages
	...
	if [ -n "$packages" ] && [ -e "$target-src/bin/$target/packages/base" ]; then # remove associated bin files only
		for package in $packages ; do
				$(find "$target-src/bin/$target/packages/base" -name "$package\_*.ipk" -type f -print | xargs /bin/rm -f)
		done
	else
		packages=$(ls "$package_dir" ) # consider all gargoyle packages
		rm -rf "$target-src/bin"
	fi

	#copy gargoyle-specific packages to build directory
	for gp in $packages ; do
		IFS_ORIG="$IFS"
